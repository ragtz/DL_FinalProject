...
def batch_data_array(raw_data, batch_size, num_steps):
    ...
    # reshape to batches
    X = (np.reshape(np.transpose(X), [epoch_size, batch_size, num_steps, rows]) - 127.5)/255.0
    Y = (np.reshape(np.transpose(Y), [epoch_size, batch_size, num_steps, rows]) - 127.5)/255.0
