...
tf.app.flags.DEFINE_string('dtype', 'conv', 'Model config')
...
def main(argv):
    FLAGS.dtype = 0 if FLAGS.dtype == 'conv' else 1
    ...
    print "Init Model"
    lstmgan_model = LSTMGANModel(config.configs[FLAGS.config], lstmgan_input, session, FLAGS.logdir, FLAGS.dtype)
    session.run(tf.initialize_all_variables())

    saver = tf.train.Saver(tf.all_variables(), max_to_keep=1000)

    lstmgan_model.train(saver, FLAGS.model, FLAGS.save_iter, FLAGS.test_iter)
