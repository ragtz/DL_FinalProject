...
class LSTMModelHalf(object):
    def __init__(self, config, lstm_input, session, summary_dir, name="lstm_model"):
        ...
        with tf.variable_scope(self.scope):
            ...
            tf.image_summary('train_img', tf.reshape(255*tf.transpose(self.ybatch, perm=[0,2,1]), [self.config.batch_size, self.lstm_input.feature_vector_size, self.config.num_steps, 1]), max_images=10)
    ...
    def test(self):
        ...
        Y = self.lstm_input.test_Y
        initial_state = np.zeros((self.lstm_input.test_size, 2*self.config.num_layers*self.config.hidden_size))
        _, next_lstm_state = self.session.run([self.final_outputs, self.lstm_new_state], feed_dict = {self.xbatch: X, self.initial_state: initial_state})
        ...
        return tf.reduce_mean(tf.nn.l2_loss(tf.sub(gen_Y_reshaped, Y_reshaped))).eval(session=self.session)
