...
class LSTMGANModel(object):
    ...
    def get_var_names(self, gv):
        return tf.pack([v.name for _, v in gv], axis=0)

    def get_var_norms(self, gv):
        return tf.pack([tf.nn.l2_loss(v) for _, v in gv], axis=0)

    def get_grad_norms(self, gv):
        return tf.pack([tf.nn.l2_loss(g) for g, _ in gv], axis=0)
    ...
    def train_batch(self, xbatch, ybatch):
        ...
        # train discriminator
        d_loss, _, d1_outputs, d2_outputs, summary, v_nms, g_nms = self.session.run([self.d_loss, self.train_d, self.d1_outputs, self.d2_outputs, self.summary, self.var_norms, self.grad_norms], feed_dict={self.xbatch: xbatch, self.ybatch: ybatch, self.initial_state: initial_state})
